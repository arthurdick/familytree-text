...
Files begin with global metadata keys _before_ the first `ID:` anchor is declared.
...
- **Examples:**
    _ `DOUGLAS-1904` (Latin)
    _ `ГОРБАЧЕВ-1931` (Cyrillic)
    _ `MÜLLER-1890` (Latin Extended)
    _ `田中-1950` (CJK)
    _ `Smith_John_1` (Underscore)
    _ `J.Doe.1990` (Period)
...
1. **Manifest Match:** If a record listed in the `CHILD` manifest matches a record that _also_ claims this individual as a `PARENT`, it is placed in the list at the explicit position defined.
2. **Append:** Any verified children discovered via scan (Section 4.2.1) that are _not_ present in the `CHILD` manifest must be appended to the end of the list, sorted chronologically by birth date.
...
- Is the line empty or does it contain _only_ whitespace?
- **If YES:**
...
- _Note:_ Do not close the buffer. The block is still active.
...
- _Space Folding Rule:_ Check the last character in the current buffer. If it is not a newline marker, append a single **Space character**.
...
- _Rationale:_ A duplicate ID usually indicates a copy-paste error where the user intended to create a new record but forgot to update the identifier. Continuing to parse (or silently ignoring the block) risks discarding valuable data or corrupting the graph.

3. **Error Reporting:** The error message must explicitly identify:

- The **Conflicting ID**.
- The **Line Number** of the _original_ definition.
- The **Line Number** of the _duplicate_ attempt.
...
- _Rule:_ You cannot cite a source or link to an event that does not exist.
...
- _Runtime Behavior:_ Treat as a valid edge to a "Null/Unknown" node.
...
| _Empty_ | **Standard**  | No special status (Default).                                    |
